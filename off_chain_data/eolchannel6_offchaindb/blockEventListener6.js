...
async function main() {
    try {
        ...
        const userExists = await wallet.get('appUser6');
        if (!userExists) {
            console.log('An identity for the user "appUser6" does not exist in the wallet');
        ...
        const network = await gateway.getNetwork('eolchannel6');
