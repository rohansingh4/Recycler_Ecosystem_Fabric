...
approveForMyOrg1() {
    ...
    peer lifecycle chaincode approveformyorg -o localhost:8050 \
        --ordererTLSHostnameOverride orderer2.example.com --tls \
        --collections-config $PRIVATE_DATA_CONFIG \
...
getBlock() {
    setGlobalsForPeer0ServiceOrg3
    # peer channel fetch 10 -c mychannel -o localhost:8050 \
    #     --ordererTLSHostnameOverride orderer.example.com --tls \
    #     --cafile $ORDERER_CA

    peer channel getinfo  -c mychannel -o localhost:8050 \
        --ordererTLSHostnameOverride orderer2.example.com --tls \
        --cafile $ORDERER_CA
}
...
#     peer lifecycle chaincode approveformyorg -o localhost:8050 \
#         --ordererTLSHostnameOverride orderer2.example.com --tls $CORE_PEER_TLS_ENABLED \
...
commitChaincodeDefination() {
    setGlobalsForPeer0ServiceOrg3
    peer lifecycle chaincode commit -o localhost:8050 --ordererTLSHostnameOverride orderer2.example.com \
...
chaincodeInvoke() {
    # setGlobalsForPeer0Org1
    # peer chaincode invoke -o localhost:8050 --ordererTLSHostnameOverride orderer2.example.com \
